...
// Create a portal target div in the document body
...
export const DeleteButton: React.FC<{
  ...
  React.useEffect(() => {
    ...
    const root = document.getElementById('modal-root') || createPortalRoot();
  ...
  return (
    <>
      ...
      {showConfirm && ReactDOM.createPortal(
        ...
        >
          <div 
            className="bg-white dark:bg-slate-800 rounded-xl p-5 w-full max-w-xs mx-4 shadow-2xl border-2 border-red-200 dark:border-red-800 relative"
            style={{
          ...
          >
            ...
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">Delete Chat?</h3>
              <p className="text-slate-500 dark:text-slate-400 text-xs mb-3">This action cannot be undone</p>
              
              <p className="text-slate-600 dark:text-slate-300 mb-4 text-sm">
                Delete <span className="text-red-600 dark:text-red-400 font-medium">"{sessionTitle}"</span>?
                <span className="block text-xs text-slate-500 dark:text-slate-400 mt-1">All messages will be removed</span>
              </p>
              
              <div className="flex space-x-2">
                <button
                  onClick={() => setShowConfirm(false)}
                  className="flex-1 px-3 py-1.5 text-xs bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors font-medium"
                >
                  Cancel
                </button>
                <button
